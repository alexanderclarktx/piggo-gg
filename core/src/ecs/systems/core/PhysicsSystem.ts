...
import { ClientSystemBuilder, Collider, Entity, Position, SystemBuilder, XYdistance, abs, entries, keys, round, sign } from "@piggo-gg/core"

export let physics: RapierWorld
RapierInit().then(() => physics = new RapierWorld({ x: 0, y: 0 }))

export const LocalPhysics = ClientSystemBuilder({
  id: "LocalPhysics",
  init: (world) => {
